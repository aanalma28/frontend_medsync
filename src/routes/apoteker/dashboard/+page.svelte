<script lang="ts">
	...
	let prescriptions = $state([
		...
		{
			id: 2,
			patient: 'Bapak Sudirman',
			doctor: 'dr. Arif',
			time: '08:50 WIB',
			waitTime: '10 mnt',
			status: 'Menunggu' as PrescriptionStatus,
			isUrgent: false,
			medicines: ['Isosorbide Dinitrate 5mg']
		},
		{
			id: 3,
	...
	let filteredPrescriptions = $derived(
		prescriptions.filter(
			(item) =>
				...
				item.medicines.some((m) => m.toLowerCase().includes(searchQuery.toLowerCase()))
		)
	);

	// Mengambil resep prioritas pertama (yang Urgent dan Menunggu)
	let urgentPrescription = $derived(
		prescriptions.find((p) => p.status === 'Menunggu' && p.isUrgent) ||
			prescriptions.find((p) => p.status === 'Menunggu')
	);
	...
	function updateStatus(id: number, status: PrescriptionStatus) {
		...
		if (
			status === 'Siap ambil' &&
			!pickupQueue.find((p) => p.patient === prescriptions.find((x) => x.id === id)?.patient)
		) {
			const p = prescriptions.find((x) => x.id === id)!;
			pickupQueue = [
				...pickupQueue,
				{ patient: p.patient, medicine: p.medicines[0], time: 'Baru Saja Siap' }
			];
		}
	}
...
<Title title="Apoteker | Dashboard" />
	...
	<main class="flex h-full flex-1 flex-col overflow-hidden">
		...
		<div class={!isForbidden ? 'flex-1 overflow-y-auto px-5 py-6 md:px-8 lg:px-10 lg:py-10' : ''}>
			...
			{:else}
				...
				<div
					class="relative mb-8 overflow-hidden rounded-[24px] bg-gradient-to-br from-slate-900 via-stone-900 to-amber-900 p-6 text-white shadow-xl sm:p-8"
				>
					<div
						class="absolute -top-10 -right-10 h-40 w-40 rounded-full bg-amber-500/20 blur-3xl"
					></div>
						...
						<div>
							<div class="mb-3 flex items-center gap-2">
								<span class="relative flex h-2.5 w-2.5"
									...
									></span></span
								>
								<p class="text-[11px] font-bold tracking-widest text-amber-200 uppercase">
									Sistem Farmasi Aktif
								</p>
							</div>
							...
							<p class="mt-2 text-sm text-slate-300 sm:text-base">
								Ada <strong class="text-white"
									>{prescriptions.filter((p) => p.status === 'Menunggu').length} resep</strong
								>
								menunggu diracik, dan
								<strong class="text-white">{pickupQueue.length} pasien</strong> di loket.
							</p>
						</div>
						<div
				...
				{#if activeMenu === 'beranda'}
					...
					<section class="mb-6 grid gap-4 sm:grid-cols-3">
						...
						<div class="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
							...
							<p class="mt-2 text-3xl font-black text-emerald-600">
								{prescriptions.filter((p) => p.status === 'Siap ambil').length}
							</p>
						</div>
					...
					{#if urgentPrescription}
						<section
							class="mb-6 rounded-[24px] border-2 border-rose-500 bg-rose-50 p-6 shadow-lg sm:p-8"
						>
							<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
								<div>
									<div class="mb-2 flex items-center gap-2">
										<span
											class="rounded bg-rose-200 px-2 py-1 text-[10px] font-black tracking-wider text-rose-800 uppercase"
											>Prioritas Racik</span
										>
										<span class="text-sm font-bold text-rose-600"
											>Menunggu {urgentPrescription.waitTime}</span
										>
									</div>
									<h2 class="text-2xl font-black text-slate-900">{urgentPrescription.patient}</h2>
									<p class="mt-1 text-sm font-medium text-slate-600">
										Dari: {urgentPrescription.doctor}
									</p>
								</div>
								<div class="rounded-xl bg-white p-4 shadow-sm md:w-1/2">
									<p class="text-xs font-bold text-slate-400 uppercase">
										Daftar Obat (Tindakan Segera)
									</p>
									<div class="mt-2 flex flex-wrap gap-2">
										{#each urgentPrescription.medicines as med (med)}
											<span
												class="rounded-lg border border-slate-200 bg-slate-100 px-3 py-1 text-sm font-bold text-slate-800"
												>{med}</span
											>
										{/each}
									</div>
									<button
										onclick={() => updateStatus(urgentPrescription.id, 'Siap ambil')}
										class="mt-4 w-full rounded-xl bg-rose-600 py-2.5 text-sm font-bold text-white shadow-md transition hover:bg-rose-700"
										>Tandai Selesai Diracik</button
									>
								</div>
							</div>
						</section>
					...
					<div class="grid gap-6 xl:grid-cols-[1.25fr_0.8fr]">
						...
						<section class="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
							<h2 class="mb-5 border-b border-slate-100 pb-4 text-xl font-bold text-slate-900">
								Semua Antrean Resep
							</h2>

							<input
								bind:value={searchQuery}
								class="mb-5 w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm transition outline-none focus:border-amber-500 focus:bg-white focus:ring-2 focus:ring-amber-100"
							...
							<div class="space-y-4">
								{#each filteredPrescriptions as p (p.id)}
									<div
										class={`flex flex-col gap-4 rounded-xl border-l-4 bg-slate-50 p-4 shadow-sm sm:flex-row sm:justify-between ${p.status === 'Menunggu' ? (p.isUrgent ? 'border-l-rose-500' : 'border-l-amber-400') : 'border-l-emerald-400'}`}
									>
										<div>
											<div class="mb-1 flex items-center gap-2">
												<p class="font-bold text-slate-900">{p.patient}</p>
												<span
													class={`rounded px-1.5 py-0.5 text-[10px] font-black tracking-wider uppercase ${p.status === 'Menunggu' ? 'bg-amber-200 text-amber-800' : 'bg-emerald-200 text-emerald-800'}`}
													>{p.status}</span
												>
											</div>
											<p class="text-xs font-medium text-slate-500">
												Waktu: {p.time} • Dokter: {p.doctor}
											</p>
											<div class="mt-2 flex flex-wrap gap-1">
												{#each p.medicines as med (med)}
													<span
														class="rounded border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-bold text-slate-700"
														>{med}</span
													>
												{/each}
											</div>
										...
										<div class="flex flex-col justify-center">
											{#if p.status === 'Menunggu'}
												<button
													onclick={() => updateStatus(p.id, 'Siap ambil')}
													class="rounded-xl border border-amber-300 bg-white px-4 py-2 text-xs font-bold text-amber-700 hover:bg-amber-50"
													>Selesai Racik</button
												>
											{:else if p.status === 'Siap ambil'}
												<button
													onclick={() => updateStatus(p.id, 'Selesai')}
													class="rounded-xl bg-emerald-500 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-600"
													>Diserahkan</button
												>
											{/if}
										</div>
								...
								<div class="mt-4 space-y-3">
									{#each pickupQueue as item (item.patient)}
										...
										>
											...
											<button
												class="rounded-lg bg-emerald-500 px-3 py-1.5 text-xs font-bold hover:bg-emerald-400"
												>Panggil</button
											>
										</div>
									{/each}
									{#if pickupQueue.length === 0}
										<p class="py-4 text-center text-sm text-slate-500">Loket kosong</p>
									{/if}
								</div>
							</div>
									...
									{#each inventory.filter((i) => i.quantity < 10) as item (item.category)}
										...
										>
											<div>
												...
												<p class="text-[10px] font-black text-rose-600 uppercase">{item.note}</p>
											</div>
				...
				{:else if activeMenu == 'users'}
					<div class="space-y-6">
						<!-- Header & Pencarian -->
						<div
							class="flex flex-col gap-4 rounded-[24px] border border-amber-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-8"
						>
							<div>
								<h2 class="text-xl font-bold text-slate-900">Direktori & Verifikasi Pasien</h2>
								<p class="mt-1 text-sm text-slate-500">
									Cari data pasien untuk verifikasi pengambilan resep dan validasi nomor BPJS.
								</p>
							</div>
							...
							<div class="w-full sm:w-72">
								<input
									...
									class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm transition outline-none focus:border-amber-500 focus:bg-white focus:ring-2 focus:ring-amber-100"
						...
						<div class="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
							<div class="overflow-x-auto">
								<table class="w-full border-collapse text-left">
									<thead>
										<tr
											class="border-b border-slate-200 text-xs font-bold tracking-wider text-slate-400 uppercase"
										>
											<th class="px-4 pb-3">ID Pasien</th>
											<th class="px-4 pb-3">Nama Lengkap</th>
											<th class="px-4 pb-3">No. Telepon / WhatsApp</th>
											<th class="px-4 pb-3">No. BPJS / Asuransi</th>
									...
									<tbody class="divide-y divide-slate-100 text-sm">
										{#each filteredUsers as user (user.id)}
											<tr class="transition hover:bg-slate-50">
												...
												<td class="px-4 py-4 text-slate-600">{user.bpjs}</td>
												<td class="px-4 py-4">
													<span
														class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-800"
														>{user.status}</span
													>
												</td>
												<td class="px-4 py-4 text-right">
													<button
														class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 shadow-sm hover:bg-slate-50"
														>Verifikasi</button
													>
												</td>
											</tr>
										{:else}
											<tr>
												<td colspan="6" class="py-8 text-center text-slate-400"
													>Tidak ada pasien yang ditemukan.</td
												>
											</tr>
										{/each}
									</tbody>
